...
def main():
	# Prepare the data
	dataset = pd.read_csv("/home/ubuntu/DeepDifE/diatoms_data/high_light_30min_genes_lfc_cutoff_2_1000up.csv")
	dataset = dataset[~dataset.Sequence.str.contains("N")]
	...
	train_df, validation_test_df = grouped_shuffle_split(dataset, dataset["GeneFamily"], 0.3)
	...
	f = open("/home/ubuntu/DeepDifE/cytokinine_data/results/result_cytokinine_with_evo_aug.csv", "a")
	f.write("loss,accurcay,auroc,auprc,TP,test_auroc,train_auroc\n")
	f.close()
	...
	for i in range(0,10):
		...
		f = open("/home/ubuntu/DeepDifE/cytokinine_data/results/result_cytokinine_with_evo_aug.csv", "a")
		f.write(f'{val_scores[0]},{val_scores[1]},{val_scores[2]},{val_scores[3]},{val_scores[4]},{test_auroc},{train_auroc},{finetune_val_scores[0]},{finetune_val_scores[1]},{finetune_val_scores[2]},{finetune_val_scores[3]},{finetune_val_scores[4]},{finetune_test_auroc},{finetune_train_auroc}\n')
		# f.write(f'{val_scores[0]},{val_scores[1]},{val_scores[2]},{val_scores[3]},{val_scores[4]},{test_auroc},{train_auroc}\n')
		f.close()
