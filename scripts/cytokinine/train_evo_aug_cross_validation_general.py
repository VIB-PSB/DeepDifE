...
from src.diff_expression_model import compile_model
from src.prepare_dataset import one_hot_encode_series, reverse_complement_series, reverse_complement_sequence, prepare_dataset_from_fasta, grouped_shuffle_split
...
def run(fasta_path, output, datapath):
	# Prepare the data
	...
	train_df = prepare_dataset_from_fasta( fasta_path=fasta_path,
							dataset=train_df,
...
def main():
	parser = argparse.ArgumentParser(description='General evo aug cross validation script. Give the correct fasta file you want to train on')
	parser.add_argument('-f','--fasta', help='Fasta file containing the sequences', required=True)
	parser.add_argument('-o','--output', help='Result output', required=True)
	parser.add_argument('-d','--datapath', help='Path where the dataset will be stored', required=True)

	args = vars(parser.parse_args())
	run(args["fasta"], args["output"], args["datapath"])
